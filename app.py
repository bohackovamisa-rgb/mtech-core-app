...
# ==================== TAB 1: LEAN CANVAS ====================
with tab_canvas:
    st.info("💡 **Lean Canvas je dynamický nástroj**, který se vyvíjí a mění v závislosti na životním cyklu vašeho produktu. Můžete si jej sice ručně vyplňovat a přepisovat sami v záložce Firemní Dashboard, ale pokud jej do Dashboardu **propíšete odsud** (až po úspěšném obhájení a splnění 80 % u Mentora), získáte mnohem větší jistotu, že je váš byznys model skutečně reálný a životaschopný.")
    ...
    if val_score >= 80 and not is_teacher:
        ...
        if st.button("Zapsat a přepsat ruční Lean Canvas v Dashboardu", type="primary"):
            # 1. Zápis do popisu firmy (pro Zakladatelský spis)
            spis_text = (
                f"**NAŠE ŘEŠENÍ (MVP):**\n{canvas.get('reseni', '')}\n\n"
                f"**ŘEŠÍME PROBLÉM:**\n{canvas.get('problem', '')}\n\n"
                f"**CÍLOVÁ SKUPINA:**\n{canvas.get('cilovka', '')}\n\n"
            ...
            # 2. Přepis 9 políček Lean Canvasu
            lc_payload = {
                "problem": canvas.get("problem", ""),
                "reseni": canvas.get("reseni", ""),
                "hodnota": canvas.get("hodnota", ""),
                "nefer_vyhoda": canvas.get("nefer_vyhoda", ""),
                "cilovka": canvas.get("cilovka", ""),
                "metriky": canvas.get("metriky", ""),

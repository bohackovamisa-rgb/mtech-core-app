...
st.markdown("""
    ...
    .login-box { background-color: #1e293b; padding: 40px; border-radius: 16px; border: 1px solid #334155; text-align: center; }
...
if not st.session_state.prihlasen:
    ...
    with col1:
        st.markdown("<div class='feature-box'><h4>Pro Žáky</h4><p style='font-size: 0.9em; color: #cbd5e1;'>Získávají M-Kredity za plnění úkolů a učí se pracovat s digitální peněženkou.</p></div>", unsafe_allow_html=True)
    with col2:
        st.markdown("<div class='feature-box'><h4>Pro Firmy</h4><p style='font-size: 0.9em; color: #cbd5e1;'>Vypisují zakázky, nabízí produkty a spravují kapitál v dashboardu.</p></div>", unsafe_allow_html=True)
    with col3:
        st.markdown("<div class='feature-box'><h4>Pro Školy</h4><p style='font-size: 0.9em; color: #cbd5e1;'>Kontrolní úřad s dohledem nad transakcemi a ekonomikou.</p></div>", unsafe_allow_html=True)

    st.write("---")

    tab_login, tab_user_reg, tab_school_licence = st.tabs(["Přihlášení do systému", "Registrace (Kód školy)", "Objednávka Licence"])
                ...
                if jmeno and heslo:
                    try:
                        ...
                        if isinstance(data, list) and len(data) > 0:
                            ...
                            st.session_state.role = str(uzivatel["role"]).lower()
        ...
        with st.form("school_form"):
            ...
            if st.form_submit_button("Odeslat nezávaznou poptávku"):
                if nazev_skoly and email:
                    ...
                    requests.post(f"{SUPABASE_URL}/rest/v1/licencovane_skoly", headers=headers, json={"nazev_skoly": nazev_skoly, "kontaktni_email": email, "licencni_kod": generovany_kod, "max_firem": pocet_firem, "max_zaku": pocet_zaku, "zaplaceno": False})
                    st.success(f"Děkujeme! Poptávka zaznamenána pod kódem **{generovany_kod}**.")
                else:

...
tab_zalozeni, tab_brand, tab_vyvoj, tab_hr, tab_kalkulace, tab_ucto, tab_burza = st.tabs([
    "1. Zakladatelský Spis", "2. Brand a AI Mentor", "3. Agilní Vývoj", "4. Tým a HR", "5. Cenotvorba", "6. Účetnictví a Materiál", "7. Burza"
])
...
with tab_zalozeni:
    ...
    if moje_firma:
        ...
        st.download_button(label="Stáhnout Listinu", data=doc, file_name=f"Spis_{moje_firma['nazev_firmy']}.md", mime="text/markdown", icon=":material/download:")
    ...
    if not moje_firma or st.session_state.get("edit_spis", False):
        ...
        with u_financak:
            st.session_state.reg_data["typ_dani"] = st.multiselect("Daně:", ["DPPO", "Závislá činnost", "Nezávislá činnost", "M-TECH daň"], default=["DPPO"])
    ...
    if moje_firma:
        ...
        with tab_aktiva:
            with st.form("form_brand"):
                ...
                if st.form_submit_button("Uložit", icon=":material/save:"):
        ...
        with tab_lean:
            ...
            with st.form("form_canvas"):
                ...
                if st.form_submit_button("Uložit Canvas", icon=":material/save:"):
                    ...
                    st.rerun()
        with tab_ai:
            if st.button("Spustit AI Audit projektu"):
                score = 0
                if moje_firma.get("logo_url") and moje_firma.get("web_url"): score += 25
                if exist_canvas and exist_canvas.get("problem") and exist_canvas.get("reseni"): score += 35
                res_k = requests.get(f"{SUPABASE_URL}/rest/v1/kalkulacni_listy?firma_id=eq.{moje_firma['id']}", headers=headers).json()
                if res_k: score += 40
                st.markdown(f"<div class='card-box'><h3>Skóre: {score} %</h3></div>", unsafe_allow_html=True)

with tab_vyvoj:
    if moje_firma:
        ag_kanban, ag_porady, ag_outsourcing = st.tabs(["Agilní Kanban", "Porady", "B2B Outsourcing"])
        with ag_kanban:
            with st.form("form_novy_ukol"):
                ...
                if st.form_submit_button("Přidat", icon=":material/add:"):
            ...
            with col_todo:
                ...
                for u in [x for x in ukoly if x['stav'] == 'TO_DO']:
                    ...
                    if st.button("▶️", key=f"btn_ip_{u['id']}"):
            ...
            with col_ip:
                ...
                for u in [x for x in ukoly if x['stav'] == 'IN_PROGRESS']:
                    st.markdown(f"<div class='kanban-card' style='border-color:#f59e0b;'><h5>{u['nazev_ukolu']}</h5></div>", unsafe_allow_html=True)
                    if st.button("✅", key=f"btn_done_{u['id']}"):
            ...
            with col_done:
                ...
                for u in [x for x in ukoly if x['stav'] == 'DONE']:
                    st.markdown(f"<div class='kanban-card' style='border-color:#10b981;'><h5>{u['nazev_ukolu']}</h5></div>", unsafe_allow_html=True)
                    if st.button("🗑️", key=f"btn_del_{u['id']}"):
        ...
        with ag_outsourcing:
            ...
            with st.form("form_outsourcing"):
                dodavatel_nazev = st.selectbox("Dodavatel:", [f['nazev_firmy'] for f in ostatni_firmy] if ostatni_firmy else ["Nikdo"])
                o_predmet = st.text_input("Předmět:")
                o_cena = st.number_input("Částka (M-K):", min_value=1.0, value=50.0)
                if st.form_submit_button("Zaplatit"):
                    ...
                    if dodavatel_firma and o_predmet:
                        ...
                        requests.post(f"{SUPABASE_URL}/rest/v1/kniha_prijmu_vydaju", headers=headers, json={"firma_id": dodavatel_firma["id"], "typ_transakce": "PRIJEM", "titul": f"B2B zakázka", "castka": o_cena, "auditovano": False})
...
with tab_hr:
    if moje_firma:
        ...
        st.write("---")
        st.markdown(f"#### Zúčtování mzd (Daň: {akt_dan_prijem} %)")
        res_z = requests.get(f"{SUPABASE_URL}/rest/v1/zamestnanci?firma_id=eq.{moje_firma['id']}&select=*", headers=headers).json()
        if res_z:
            vybrany_z_jmeno = st.selectbox("Výplata mzdy:", [z["jmeno_zamestnance"] for z in res_z])
            ...
            if vybrany_z:
                ...
                st.info(f"Hrubá: {hruba} M-K | Daň: {dan_castka} M-K | Čistá: {cista} M-K (cca {cista * kurz_kc:,.0f} Kč)")
                
                if st.button("Odeslat výplatu a odvést daň"):
                    ...
                    if res_ceo and hruba <= res_ceo[0]['kredity']:
                        requests.patch(f"{SUPABASE_URL}/rest/v1/uzivatele?jmeno=eq.{moje_firma['ceo_jmeno']}", headers=headers, json={"kredity": res_ceo[0]['kredity'] - hruba})
                        ...
                        st.rerun()
                    else: st.error("Nedostatek peněz na účtu!")
...
with tab_kalkulace:
    if moje_firma:
        ...
        with col_c_left:
            ...
            st.caption(f"Sazba M-TECH Daně: **{akt_dan_mtech} %**")
            with st.form("form_kalkulace"):
                ...
                col_k1, col_k2 = st.columns(2)
                with col_k1: p_naklady = st.number_input("Přímé náklady (M-K):", min_value=0.0, value=35.0)
                with col_k2: marze = st.number_input("Zisk (M-K):", min_value=0.0, value=50.0)
                k_cena = (p_naklady + marze) * (1 + (akt_dan_mtech / 100.0))
                st.markdown(f"**Koncová cena pro trh:** `{k_cena:.2f} M-Kreditů` *(odpovídá cca {k_cena * kurz_kc:,.0f} Kč)*")
    ...
    if moje_firma:
        ...
        with col_u1:
            ...
            with st.form("form_nakup_materialu"):
                titul_nakupu = st.text_input("Předmět nákupu (např. 2x Filament, Hodina 3D tisku):")
                castka_nakup = st.number_input("Celková cena dle Ceníku (M-K):", min_value=1.0, value=10.0)
                st.caption(f"Reálná hodnota nákupu: {castka_nakup * kurz_kc:,.0f} Kč")
                if st.form_submit_button("Zaplatit škole", icon=":material/shopping_cart:"):
                    res_ceo = requests.get(f"{SUPABASE_URL}/rest/v1/uzivatele?jmeno=eq.{moje_firma['ceo_jmeno']}", headers=headers).json()
        ...
        with col_u2:
            st.subheader("Žádost o Bankovní úvěr")
            with st.form("form_uver"):
                u_castka = st.number_input("Požadovaná částka (M-K):", min_value=50, value=500)
                u_ucel = st.text_input("Účel půjčky:")
                if st.form_submit_button("Odeslat do banky", icon=":material/account_balance:"):
            ...
            if uvery:
                for uv in uvery:
                    if uv['stav'] == 'ZADOST': st.markdown(f"<div class='card-box'><p>Žádost o {uv['castka']} M-K</p><span class='status-wait'>Čeká</span></div>", unsafe_allow_html=True)
                        ...
                        with st.form(f"splatka_{uv['id']}"):
                            ...
                            if st.form_submit_button("Odeslat splátku"):
                                res_ceo = requests.get(f"{SUPABASE_URL}/rest/v1/uzivatele?jmeno=eq.{moje_firma['ceo_jmeno']}", headers=headers).json()
                                if res_ceo and splatka <= res_ceo[0]['kredity']:
                                    requests.patch(f"{SUPABASE_URL}/rest/v1/uzivatele?jmeno=eq.{moje_firma['ceo_jmeno']}", headers=headers, json={"kredity": res_ceo[0]['kredity'] - splatka})
                                    requests.post(f"{SUPABASE_URL}/rest/v1/kniha_prijmu_vydaju", headers=headers, json={"firma_id": moje_firma["id"], "typ_transakce": "VYDAJ", "titul": "Splátka úvěru", "castka": splatka, "auditovano": True})
    ...
    if moje_firma:
        ...
        with col_b2:
            ...
            with st.form("form_dividendy"):
                ...
                if st.form_submit_button("Vyplatit"):

...
st.markdown("""
    ...
    .asset-link { color: #10b981; font-weight: bold; text-decoration: none; }
    .asset-link:hover { text-decoration: underline; }
    </style>
""", unsafe_allow_html=True)

st.title("🚀 Startup Hub M-TECH CORE")
st.caption("Buduj. Inovuj. Škáluj. Tvoje cesta od nápadu v garáži až po plně funkční byznys a reálný produkt/službu.")
...
exist_canvas = None
has_canvas, has_porada, has_kalkulace, has_ucto, has_brand = False, False, False, False, False
...
if moje_firma:
    ...
    with col_s2:
        if has_brand and has_canvas: st.markdown('<div class="status-badge-ok">Brand & Pitch 🎨</div>', unsafe_allow_html=True)
        else: st.markdown('<div class="status-badge-off">No Brand Identity</div>', unsafe_allow_html=True)
    with col_s3:
        if has_porada: st.markdown('<div class="status-badge-ok">Roadmap Active ⚡</div>', unsafe_allow_html=True)
        else: st.markdown('<div class="status-badge-off">Bez Projektu</div>', unsafe_allow_html=True)
    with col_s4:
...
with tab_zalozeni:
    ...
    if not moje_firma or st.session_state.get("edit_spis", False):
        ...
        with u_notar:
            ...
            with col_n1:
                st.session_state.reg_data["ceo"] = st.text_input("1.4 CEO (Visionary):", value=st.session_state.reg_data.get("ceo", moje_firma['ceo_jmeno'] if moje_firma else uzivatel))
                st.session_state.reg_data["cfo"] = st.text_input("1.5 CFO (Finance):", value=st.session_state.reg_data.get("cfo", moje_firma['cfo_jmeno'] if moje_firma else ""))
            with col_n2:
                st.session_state.reg_data["cto"] = st.text_input("1.6 CTO (Tech / Product):", value=st.session_state.reg_data.get("cto", moje_firma['cto_jmeno'] if moje_firma else ""))
        ...
        with u_zivnost:
            ...
            with col_j1:
                st.session_state.reg_data["divize"] = st.selectbox("2.1 Divize:", ["Hardware", "Energy", "Software & AI", "Services & Market"])
            with col_j2:
                st.session_state.reg_data["predmet"] = st.text_input("2.2 Core Business:", value=st.session_state.reg_data.get("predmet", ""))
            st.session_state.reg_data["zamer"] = st.text_area("2.3 Detailní popis:", value=st.session_state.reg_data.get("zamer", ""))

        with u_financak:
            st.session_state.reg_data["dan_souhlas"] = st.checkbox("3.1 Zavazujeme se k odvodu M-TECH daně z profitu.", value=st.session_state.reg_data.get("dan_souhlas", True))
        ...
        with u_rejstrik:
            ...
            if st.button("🚀 ODESLAT DO AKCELERÁTORU (Úřad)", icon=":material/send:"):
...
# ==========================================
# TAB 2: BRAND, VISION & PITCH
# ==========================================
with tab_canvas:
    if not moje_firma:
        st.warning("Nejprve musíte projít procesem Založení (Legal).")
    else:
        st.subheader("Brand Kit & Startup Vision")
        st.caption("Firma bez identity neexistuje. Vložte odkazy na vaše reálné logo, webovky a vizuální identitu, a sestavte Lean Canvas.")
        
        tab_brand, tab_lean = st.tabs(["🎨 Vizuální Identita (Brand Kit)", "💡 Lean Canvas (Byznys model)"])
        
        with tab_brand:
            with st.form("form_brand"):
                st.info("💡 **Tip:** Reálně vygenerujte Logo v Canvě, Web ve Webnode/Wix, nebo Leták ve Figmě. Sem vložte POUZE odkazy pro investory/vyučujícího.")
                b_logo = st.text_input("🔗 Odkaz na firemní LOGO (Google Drive, Canva, imgur):", value=moje_firma.get('logo_url','') or "")
                b_web = st.text_input("🔗 Odkaz na WEBOVÉ STRÁNKY (nebo Instagram / sociální sítě):", value=moje_firma.get('web_url','') or "")
                b_promo = st.text_input("🔗 Odkaz na LETÁK / PITCH DECK (Promo materiály):", value=moje_firma.get('promo_url','') or "")
                
                if st.form_submit_button("Aktualizovat Brand Kit", icon=":material/brush:"):
                    ...
                    st.success("Brand identita uložena!")
                    st.rerun()
            
            if moje_firma.get("logo_url") or moje_firma.get("web_url"):
                st.markdown("### Vaše Aktiva:")
                if moje_firma.get("logo_url"): st.markdown(f"🎨 **Logo:** <a href='{moje_firma['logo_url']}' class='asset-link' target='_blank'>Zobrazit logo</a>", unsafe_allow_html=True)
                if moje_firma.get("web_url"): st.markdown(f"🌐 **Web:** <a href='{moje_firma['web_url']}' class='asset-link' target='_blank'>Přejít na web</a>", unsafe_allow_html=True)
                if moje_firma.get("promo_url"): st.markdown(f"📄 **Promo:** <a href='{moje_firma['promo_url']}' class='asset-link' target='_blank'>Zobrazit leták / Pitch Deck</a>", unsafe_allow_html=True)

        with tab_lean:
            with st.form("form_canvas"):
                col_c1, col_c2, col_c3, col_c4 = st.columns(4)
                with col_c1: prob = st.text_area("1. Pain Point (Problém trhu)", value=exist_canvas.get("problem","") if exist_canvas else "", height=150)
                with col_c2: sol = st.text_area("2. Náš Produkt / Služba", value=exist_canvas.get("reseni","") if exist_canvas else "", height=150)
                with col_c3: val = st.text_area("3. Unfair Advantage", value=exist_canvas.get("unikatni_hodnota","") if exist_canvas else "", height=150)
                with col_c4: target = st.text_area("4. Target Audience", value=exist_canvas.get("cilova_skupina","") if exist_canvas else "", height=150)
                col_c5, col_c6 = st.columns(2)
                with col_c5: costs = st.text_area("5. Burn Rate (Náklady)", value=exist_canvas.get("nakladova_struktura","") if exist_canvas else "", height=100)
                with col_c6: rev = st.text_area("6. Revenue Streams (Příjmy)", value=exist_canvas.get("prijmove_toky","") if exist_canvas else "", height=100)
...
# ==========================================
# TAB 3: AGILNÍ VÝVOJ & ROADMAPA
# ==========================================
with tab_agile:
    if not moje_firma:
        st.warning("Nejprve musíte projít procesem Založení.")
    else:
        st.subheader("Projektové řízení – Kanban & Týmové Porady")
        st.caption("Realita není o odklikávání formulářů. Tady řídíte vývoj reálného prototypu produktu (MVP) nebo spuštění služby.")
        
        tab_board, tab_porady, tab_roadmap = st.tabs(["📋 Execution Board (Kanban)", "👥 Meeting Logs (Porady)", "🎯 Scale-up Roadmap (Level 3)"])
        
        with tab_board:
            with st.form("form_novy_ukol"):
                col_u1, col_u2, col_u3 = st.columns(3)
                with col_u1:
                    u_nazev = st.text_input("Nový Task (Např: Design prototypu v CAD, Založit IG):")
                with col_u2:
                    u_osoba = st.text_input("Owner (Zodpovědná osoba):", value=uzivatel)
                with col_u3:
        ...
        with tab_porady:
            st.markdown("**Stand-up Syncs & Dlouhodobé plánování**")
            st.caption("Setkali jste se a dohodli jste se na něčem? Vložte report z porady. Pokud máte podrobný zápis, vložte odkaz.")
            with st.form("form_porada"):
                projednano = st.text_area("O čem se reálně rozhodlo? (Agenda & Blockers):", placeholder="Odcházíme od produktu A a budeme poskytovat službu B, protože...")
                ukoly_zapis = st.text_area("Co musí kdo udělat do další porady (Akční kroky):")
                odkaz_zapis = st.text_input("🔗 Volitelný odkaz na kompletní zápis (Google Docs, Notion):")
                
                if st.form_submit_button("Uložit zápis z porady", icon=":material/post_add:"):
                    requests.post(f"{SUPABASE_URL}/rest/v1/zapisy_porady", headers=headers, json={"firma_id": moje_firma["id"], "projednane_body": f"{projednano}\n[Odkaz na plný dokument]({odkaz_zapis})" if odkaz_zapis else projednano, "ukoly_a_odpovednost": ukoly_zapis})
                    st.rerun()
            ...
            if res_p_hist.status_code == 200 and res_p_hist.json():
                for p in res_p_hist.json():
                    st.markdown(f"<div class='card-box'><small style='color:#0ea5e9;'>Porada z: {p['datum'][:10]}</small><p><b>Agenda:</b> {p['projednane_body']}</p><p><b>Závěry:</b> {p['ukoly_a_odpovednost']}</p></div>", unsafe_allow_html=True)

        with tab_roadmap:
            if moje_firma['uroven_projektu'] == 3:
                st.markdown("### 🦄 Milníky a dlouhodobý plán (Pouze pro Level 3)")
                st.caption("Firmy ve 3. Úrovni musí mít jasný výhled a milníky na čtvrtletí (Q1-Q4).")
                with st.form("form_roadmap"):
                    roadmapa = st.text_area("Harmonogram projektu (Milníky, termíny, odpovědnosti):", value=moje_firma.get('roadmap', ''), height=200, placeholder="Q1: Prototyp hotov do 30.11.\nQ2: Spuštění webu a kampaně do 15.2.\nQ3: První platící zákazníci...")
                    if st.form_submit_button("Uložit projektový plán", icon=":material/save:"):
                        requests.patch(f"{SUPABASE_URL}/rest/v1/firmy?id=eq.{moje_firma['id']}", headers=headers, json={"roadmap": roadmapa})
                        st.success("Milníky uloženy pro kontrolní úřad!")
                        st.rerun()
            else:
                st.info("Detailní Scale-up Roadmapa a řízení milníků (Gantt) je vyžadováno až od Úrovně 3 (Plná integrace).")
...
with tab_kalkulace:
    if not moje_firma:
        ...
        st.caption("Rozpad nákladů vašeho produktu nebo služby. Kolik to reálně stojí a jakou máte marži?")
        with st.form("form_kalkulace"):
            prod_nazev = st.text_input("Core Produkt / Služba (Název):")
            p_naklady = st.number_input("Přímé náklady materiálu/služby (M-K / ks):", min_value=0.0, value=35.0)

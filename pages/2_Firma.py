...
if moje_firma:
    ...
    with col_s2:
        st.markdown('<div class="status-badge-ok">Brand & Vize 🎨</div>', unsafe_allow_html=True)
    with col_s3:
        st.markdown('<div class="status-badge-ok">Agilní Vývoj ⚡</div>', unsafe_allow_html=True)
    with col_s4:
        st.markdown('<div class="status-badge-ok">HR & Úřady 👷</div>', unsafe_allow_html=True)
    with col_s5:
        st.markdown('<div class="status-badge-ok">Finance & Pricing 💸</div>', unsafe_allow_html=True)
...
# ==========================================
# TAB 1: ZAKLADATELSKÝ SPIS (KOMPLETNÍ FORMULÁŘE)
# ==========================================
with tab_zalozeni:
    ...
    if not moje_firma or st.session_state.get("edit_spis", False):
        ...
        with u_notar:
            ...
            with col_k2:
                st.session_state.reg_data["podily_popis"] = st.text_area("Rozdělení obchodních podílů (%):", value="CEO: 40 %, CFO: 30 %, CTO: 30 %", help="Specifikujte procentuální podíl na zisku a hlasovacích právech.")

        # --- 2. ŽIVNOSTENSKÝ ÚŘAD ---
        with u_zivnost:
            st.markdown("#### Jednotný registrační formulář (JRF)")
            col_j1, col_j2 = st.columns(2)
            with col_j1:
                st.session_state.reg_data["divize"] = st.selectbox("Oborový sektor:", ["Hardware & DeepTech", "Software, AI & IT Services", "GreenTech & Energy", "Služby, Design & Marketing"])
                st.session_state.reg_data["druh_zivnosti"] = st.selectbox("Druh živnosti:", ["Volná (Nevyžaduje odbornou způsobilost)", "Řemeslná (Vyžaduje výuční list/garanta)", "Vázaná (Vyžaduje odbornou certifikaci)"])
            with col_j2:
                st.session_state.reg_data["bozp_garant"] = st.text_input("Odborný zástupce (Garant živnosti):", value=uzivatel, help="Osoba odpovídající za řádný provoz a odbornost.")
                st.session_state.reg_data["sidlo"] = st.text_input("Adresa sídla startupu:", value="M-TECH Inkubátor, Učebna č. 1")
            
            st.session_state.reg_data["predmet"] = st.text_input("Předmět podnikání (Detailní popis činnosti):", value="Vývoj, výroba a prodej inovačních technologických řešení")
        ...
        with u_rejstrik:
            ...
            if st.button("🚀 ODESLAT ZAKLADATELSKÝ SPIS ÚŘADU K AUDITU", icon=":material/send:"):
                ...
                if d.get("nazev_firmy") and d.get("skolni_kod") and d.get("cfo") and d.get("cto"):
                    ...
                    zamer_str = f"[{d.get('divize')} | {d.get('druh_zivnosti')}] {d.get('predmet')} (Garant: {d.get('bozp_garant')}, Způsob jednání: {d.get('jednani')})"
...
with tab_brand:
    if not moje_firma:
        ...
        with tab_aktiva:
            st.markdown("### Firemní digitální aktiva (Brand Kit)")
            with st.form("form_brand"):
                b_logo = st.text_input("🔗 Odkaz na LOGO (Canva, Drive, Imgur):", value=moje_firma.get('logo_url','') or "")
                b_web = st.text_input("🔗 Odkaz na WEBOVÉ STRÁNKY / LANDING PAGE:", value=moje_firma.get('web_url','') or "")
                b_promo = st.text_input("🔗 Odkaz na PITCH DECK / PREZENTACI:", value=moje_firma.get('promo_url','') or "")
            ...
            if moje_firma.get("logo_url") or moje_firma.get("web_url"):
                st.markdown("---")
                if moje_firma.get("logo_url"): st.markdown(f"🎨 <a href='{moje_firma['logo_url']}' class='asset-link' target='_blank'>Zobrazit logo</a>", unsafe_allow_html=True)
                if moje_firma.get("web_url"): st.markdown(f"🌐 <a href='{moje_firma['web_url']}' class='asset-link' target='_blank'>Přejít na web</a>", unsafe_allow_html=True)
    ...
    if not moje_firma:
        ...
        with ag_kanban:
            with st.form("form_novy_ukol"):
                ...
                with col_u1: u_nazev = st.text_input("Nový úkol / User Story:")
                with col_u2: u_osoba = st.text_input("Assignee (Zodpovídá):", value=uzivatel)
            ...
            with col_todo:
                st.markdown("<div class='kanban-col-header header-todo'>BACKLOG / K VYŘEŠENÍ</div>", unsafe_allow_html=True)
            ...
            with col_ip:
                ...
                for u in [x for x in ukoly if x['stav'] == 'IN_PROGRESS']:
                    ...
                    if st.button("✅ Dokončit", key=f"btn_done_{u['id']}"):
                        ...
                        st.rerun()
            with col_done:
                st.markdown("<div class='kanban-col-header header-done'>DONE / HOTOVO</div>", unsafe_allow_html=True)
        ...
        with ag_porady:
            with st.form("form_porada"):
                projednano = st.text_area("Projednaná Agenda (Stand-up Log):")
        ...
        with ag_reporty:
            with st.form("form_report"):
                ...
                r_odkaz = st.text_input("🔗 Odkaz na soubor (Google Drive, PDF):")
...
with tab_hr:
    if not moje_firma:
        ...
        with hr_nabor:
            st.subheader("Onboarding zaměstnance & Přihláška na ČSSZ / Pojišťovnu")
            st.caption("Každý nový člen týmu musí mít uzavřenou smlouvu a být oficiálně přihlášen k pojištění.")
            ...
            with st.form("form_novy_zamestnanec"):
                ...
                with col_z2:
                    z_smlouva = st.selectbox("Typ pracovní smlouvy:", ["Pracovní smlouva (HPP)", "Dohoda o provedení práce (DPP)", "Dohoda o pracovní činnosti (DPČ)"])
                    z_sazba = st.number_input("Hodinová sazba (M-K / hod):", min_value=10, value=50)
                
                st.markdown("##### Úřední konfirmace")
                z_podpis = st.checkbox("Pracovní smlouva podepsána oběma stranami.")
                z_cssz = st.checkbox("Zaměstnanec přihlášen v registru pojištěnců (ČSSZ / ZP).")
                z_bozp = st.checkbox("Provedeno bezpečnostní školení BOZP a PO.")
                ...
                if st.form_submit_button("Registrovat nového zaměstnance", icon=":material/person_add:"):
                    if z_jmeno and z_pozice and z_podpis and z_cssz and z_bozp:
                        ...
                        st.success(f"Zaměstnanec {z_jmeno} byl úspěšně onboardován a přihlášen k pojištění!")
                        st.rerun()
                    else:
                        st.error("Vyplňte jméno, roli a potvrďte PODPIS, ČSSZ i BOZP!")
        ...
        with hr_mzdy:
            ...
            if zamestnanci_seznam:
                ...
                if vybrany_z:
                    ...
                    dan = hruba * 0.15
                    cista = hruba - dan
                    st.markdown(f"Hrubá mzda: `{hruba:.2f} M-K` | Odvod daně (15 %): `{dan:.2f} M-K` | **Čistá k výplatě: `{cista:.2f} M-K`**")
                    if st.form_submit_button("💸 Odeslat výplatu (Provést zúčtování)", icon=":material/payments:"):
...
with tab_kalkulace:
    if not moje_firma:
        ...
        with st.form("form_kalkulace"):
            ...
            obrazek = st.text_input("🔗 Odkaz na fotografii produktu (Canva, Drive, Imgur):")

...
with tab_brand:
    if not moje_firma:
        ...
        t_aktiva, t_lean, t_ai = st.tabs(["Vizuální Identita", "Lean Canvas", "AI Mentor"])
        with t_aktiva:
            with st.form("form_brand"):
                b_logo = st.text_input("Odkaz na LOGO (Obrázek):", value=moje_firma.get('logo_url','') or "")
                b_web = st.text_input("Odkaz na WEB (URL):", value=moje_firma.get('web_url','') or "")
...
# ==========================================
# 5. CENOTVORBA PRODUKTŮ PRO E-SHOP
# ==========================================
with tab_kalkulace:
    if not moje_firma:
        st.warning("Nejprve založte firmu.")

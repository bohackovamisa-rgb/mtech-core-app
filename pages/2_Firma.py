...
# Pokud není zakladatel, prověříme, zda není zaměstnanec
if not moje_firma:
...
# Zjištění přesné ROLE
moje_role = "ZAMESTNANEC"
if moje_firma:
    if moje_firma.get('ceo_jmeno', '').lower() == uzivatel.lower(): moje_role = "CEO"
    elif moje_firma.get('cfo_jmeno', '').lower() == uzivatel.lower(): moje_role = "CFO"
    elif moje_firma.get('cto_jmeno', '').lower() == uzivatel.lower(): moje_role = "CTO"
...
if moje_role == "CEO":
    ...
    # Běžný zaměstnanec / Pracovník ve výrobě
...
with tab_zalozeni:
    ...
    if not moje_firma or st.session_state.get("edit_spis", False):
        if moje_role == "CEO" or not moje_firma:
    ...
    with tab_brand:
        if not moje_firma:
            ...
            with t_ai_shark:
                ...
                posledni_pitch_dnes = any(p.get('datum', '').startswith(dnesni_datum) for p in res_pitches) if isinstance(res_pitches, list) else False
                            ...
                            if len(p_popis) < 20:
                                ...
                                if gemini_key:
                                    try:
                                        ...
                                        score_ostry = "SCHVALENO"; score_vizionarka = "ZAMITNUTO"; score_rychly = "SCHVALENO"
                                        eval_ostry = "[SCHVALENO] Ostrý: Čísla sedí."; eval_vizionarka = "[ZAMITNUTO] Vizionářová: Málo inovací."; eval_rychly = "[SCHVALENO] Rychlý: Půjdu do toho."
            ...
            with t_reklamace:
                ...
                if isinstance(reklamace_list, list) and any(r.get('datum', '').startswith(dnesni_datum) for r in reklamace_list):
        ...
        if not moje_firma:
            ...
            with ag_okr:
                ...
                with st.form("form_okr"):
                    ...
                    with col_o1: n_obj = st.text_input("Hlavní Cíl (Objective):", placeholder="Např. Ovlánout trh")
...
# ==========================================
# 8. DENÍK PRÁCE A TÝMOVÉ HODNOCENÍ (ZÁLEŽÍ NA ROLI)
# ==========================================
if tab_denik:
    with tab_denik:
        st.subheader("Deník práce a Týmové hodnocení")
        
        # Dynamické podzáložky podle vedení vs zaměstnance
        ...
        with sub_vykaz:
            ...
            st.caption(f"Přihlášený žák: **{uzivatel}** | Firma: **{moje_firma['nazev_firmy'] if moje_firma else 'Bez firmy'}**")
        ...
        # --- 3. VZÁJEMNÉ HODNOCENÍ KOLEKTIVU ---
        with sub_peer:

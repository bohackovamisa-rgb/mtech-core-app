...
st.set_page_config(page_title="Tržiště produktů", page_icon=":material/shopping_cart:", layout="wide")
...
st.markdown(f"### Zůstatek k nákupu: <span style='color:#00B4D8;'>{aktualni_kredity:.2f} M-K</span>", unsafe_allow_html=True)
st.write("---")
...
if not produkty:
    st.info("Zatím zde nejsou žádné produkty.")
else:
    cols = st.columns(3)
        ...
        with cols[index % 3]:
            st.markdown(f"""
                <div class="product-card">
                    ...
                    <div><div class="price-tag">{cena:.2f} M-K</div></div>
                </div>
            """, unsafe_allow_html=True)

...
aktualni_kredity = res_u.json()[0]['kredity'] if res_u.status_code == 200 and res_u.json() else 0
...
with tab_burza:
    ...
    st.caption("Nakupujte podíly ve firmách a získejte pasivní příjem formou dividend ze zisku.")
    
    col_b1, col_b2 = st.columns([1.5, 1])
    
    # Načtení firem pro mapování ID -> Název
    res_f = requests.get(f"{SUPABASE_URL}/rest/v1/firmy?select=id,nazev_firmy,ceo_jmeno", headers=headers)
    firmy_dict = {f['id']: f for f in res_f.json()} if res_f.status_code == 200 else {}
    ...
    with col_b1:
        ...
        if not nabidky:
            st.info("Momentálně žádná firma nenabízí své akcie.")
        else:
            for n in nabidky:
                ...
                if firma_info and n['pocet_k_prodeji'] > 0:
                    with st.container():
                        ...
                        <div class='card-box' style='border-left: 4px solid #f59e0b;'>
                            <h4>{firma_info['nazev_firmy']}</h4>
                            <p>K dispozici: <b>{n['pocet_k_prodeji']} ks</b> | Cena za akcii: <b>{n['cena_za_kus']} M-K</b></p>
                        </div>
                        """, unsafe_allow_html=True)
                        with st.form(f"buy_shares_{n['id']}"):
                            ...
                            if st.form_submit_button(f"Koupit za {celkova_cena} M-K", icon=":material/shopping_cart:"):
                                if celkova_cena > aktualni_kredity:
                                    st.error("Nedostatek prostředků!")
                                else:
                                    # 1. Strhnout peníze kupci
                                    ...
                                    # 2. Přidat peníze CEO firmy
                                    ceo = firma_info['ceo_jmeno']
                                    res_ceo = requests.get(f"{SUPABASE_URL}/rest/v1/uzivatele?jmeno=eq.{ceo}", headers=headers).json()
                                    if res_ceo:
                                        requests.patch(f"{SUPABASE_URL}/rest/v1/uzivatele?jmeno=eq.{ceo}", headers=headers, json={"kredity": res_ceo[0]['kredity'] + celkova_cena})
                                    # 3. Zapsat do portfolia
                                    ...
                                    if exist_port:
                                        nove_akcie = exist_port[0]['pocet_akcii'] + pocet_koupit
                                        requests.patch(f"{SUPABASE_URL}/rest/v1/portfolio_investoru?id=eq.{exist_port[0]['id']}", headers=headers, json={"pocet_akcii": nove_akcie})
                                    else:
                                        requests.post(f"{SUPABASE_URL}/rest/v1/portfolio_investoru", headers=headers, json={"investor_jmeno": uzivatel, "firma_id": n['firma_id'], "pocet_akcii": pocet_koupit})
                                    # 4. Ponížit nabídku
                                    ...
                                    # 5. Účetnictví firmy
                                    requests.post(f"{SUPABASE_URL}/rest/v1/kniha_prijmu_vydaju", headers=headers, json={"firma_id": n['firma_id'], "typ_transakce": "PRIJEM", "titul": f"Investice (Prodej {pocet_koupit} akcií) od {uzivatel}", "castka": celkova_cena, "auditovano": False})
                                    st.success(f"Úspěšně zakoupeno {pocet_koupit} akcií!")
                                    st.rerun()
    ...
    with col_b2:
        ...
        if not portfolio:
            st.info("Zatím nevlastníte žádné akcie.")
        else:
            for p in portfolio:
